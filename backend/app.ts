...
apiRouter.get("/test-db", async (req, res) => {
  try {
    console.log("[TEST-DB] Attempting MySQL query...");
    ...
    console.error("[TEST-DB] MySQL test failed:", err);
    res.status(500).json({
      status: "error",
      message: err.message,
    });
...
apiRouter.get("/test-bcrypt", async (req, res) => {
  try {
    ...
    console.log(
      `[TEST-BCRYPT] Hash complete in ${endHash - startHash}ms: ${hash}`,
    );
    ...
    console.log(
      `[TEST-BCRYPT] Compare complete in ${endCompare - startCompare}ms. Match: ${isMatch}`,
    );
...
apiRouter.get("/bot-setup", async (req, res) => {
  try {
    ...
    if (!bot) {
      return res
        .status(500)
        .json({ status: "error", message: "Failed to initialize bot" });
    }
